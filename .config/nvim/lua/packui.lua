...
local function git_tag(path)
	if not path or state.tags[path] ~= nil then
...
local function jump(dir)
	...
	local target
	if dir > 0 then
		target = vim.iter(plines):find(function(l)
			return l > row
		end) or plines[1]
	else
		for i = #plines, 1, -1 do
	...
	if target then
		api.nvim_win_set_cursor(state.winid, { target, 0 })
	end
end
...
local function open()
	...
	vim.keymap.set("n", "D", function()
		...
		if pdata[1] and pdata[1].active then
			vim.notify(
				("vim.pack: %s is active — remove it from your config first"):format(name),
				vim.log.levels.WARN
			)
			return
		end
